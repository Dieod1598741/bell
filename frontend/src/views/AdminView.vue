<template>
  <div class="admin-view h-full w-full flex flex-col bg-white">
    <!-- 헤더 -->
    <div class="header bg-white border-b border-gray-200 px-4 py-6">
      <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl font-bold text-gray-900 flex-1">관리자</h1>
      </div>
    </div>

    <!-- 컨텐츠 영역 -->
    <div class="flex-1 overflow-y-auto px-4 py-6">
      <div class="admin-layer max-w-4xl mx-auto">
        <!-- 탭 -->
        <div class="tabs mb-4">
          <button
            v-for="tab in tabs"
          ...
          <div class="form-content">
            ...
            <div class="form-group-with-label">
              ...
              <div class="announcement-user-list border border-gray-200 rounded-lg p-3 max-h-64 overflow-y-auto">
                <div v-if="filteredUsersForAnnouncement.length === 0" class="empty-state py-8">
                  <p class="text-gray-400 text-sm">사용자가 없습니다</p>
                </div>
                <div
                  v-for="user in filteredUsersForAnnouncement"
                  :key="user.id"
...
const filteredUsers = computed(() => {
  // admin 아이디 제외, 승인된 사용자만 (approved, admin)
  let users = allUsers.value.filter(user => 
    user.id !== 'admin' && 
    (user.permission === 'approved' || user.permission === 'admin')
  )
...
// 알람 발송용 필터링 (관리자, 승인자만, admin 아이디 제외)
const filteredUsersForAnnouncement = computed(() => {
  // 관리자(admin) 또는 승인자(approved)만, admin 아이디 제외
  let users = allUsers.value.filter(user => 
    user.id !== 'admin' && 
    (user.permission === 'admin' || user.permission === 'approved')
  )
...
const handleSelectAll = (checked) => {
  if (checked) {
    filteredUsersForAnnouncement.value.forEach(user => {
...
const toggleUserSelection = (userId) => {
  ...
  // 전체 선택 상태 업데이트
  selectAllUsers.value = filteredUsersForAnnouncement.value.length > 0 && 
    filteredUsersForAnnouncement.value.every(user => selectedUsers.value.has(user.id))
}

watch(() => filteredUsersForAnnouncement.value, () => {
